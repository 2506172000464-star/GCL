...
# CONFIG
# -------------------------------------------------
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8373191488:AAGeyXRQNOpuMhJxvsRvWfcpPqWn2dcjvpQ")
...
async def unregister_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        c = conn.cursor()
        c.execute(
            "SELECT approved, registered_at FROM players WHERE user_id=?",
            (user.id,)
        )
        row = c.fetchone()
    ...
    clear_pending_registration(user.id)
...
async def broadcast_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.reply_to_message:
        ...
        if not content:
            await msg.reply_text("No text or caption found in the replied message to broadcast.")
            return
...
def main():
    ...
    async def track_known(u: Update, c: ContextTypes.DEFAULT_TYPE):
        ensure_known_user(u.effective_user)
        ensure_known_chat(u.effective_chat)

    app.add_handler(
        MessageHandler(filters.ALL, track_known)
    )
